...
def render_html_content(
    ...
    *,
    ...
    <body>
        ...
        <div class="container">
            <div class="header">
                <div class="header-watermark">Ravenis Core</div>
        ...
        <script>
            ...
            async function saveAsImage(e) {
                ...
                try {
                    ...
                    const now = new Date();
                    const filename = `RavenisCore_热点新闻分析_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.png`;
            ...
            async function saveAsMultipleImages(e) {
                ...
                try {
                    ...
                    // 下载所有图片
                    const now = new Date();
                    const baseFilename = `RavenisCore_热点新闻分析_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
            ...
            function saveAsMarkdown() {
                ...
                lines.push('# ' + (headerTitle ? headerTitle.textContent.trim() : 'Ravenis Core'));
                lines.push('');
                ...
                // 页脚
                lines.push('---');
                lines.push('*Generated by Ravenis Core*');

                // 下载
                var md = lines.join('\\n');
                var blob = new Blob([md], { type: 'text/markdown;charset=utf-8' });
                var link = document.createElement('a');
                var filename = 'RavenisCore_' + dateStr + '_' + timeStr.replace(':', '') + '.md';
                link.download = filename;
